...
$$
\begin{equation}
    ||\textbf{X}||_{F} \coloneqq \sqrt{\sum_{i = 1}^{m} \sum_{j=1}^{n} X_{i, j}^{2}}
\end{equation}
$$
...
Let $\textbf{X} \in \mathbb{R}_{\geq 0}^{m \times n}$ be an arbitrary matrix, and let $k \in \mathbb{N}$ with $k < \min\{n, m\}$ be the number of components. 
Non-Negative Matrix Factorization (NMF or NNMF) [[2]](#lee2001) finds two matrices $\textbf{W} \in \mathbb{R}^{m \times k}_{\geq 0}$ and $\textbf{H} \in \mathbb{R}^{k \times n}_{\geq 0}$ that minimize the difference of the Frobenius norm:

$$
\begin{equation}
    ||\textbf{X} - \textbf{W}\textbf{H}||_{F}
\end{equation}
$$
...
please note that the columns of matrix $M$ are the reshaped heatmaps of each player.

Matrix $M$ is compressed using NMF resulting in the factorization
...
$$
v^{(p_{i})}_{\text{shot, cross}} = \text{concat} (\textbf{h}^\text{shot}_{i}, \textbf{h}^{cross}_{i}) \in \mathbb{R}_{\geq 0}^{12}
$$
...
Given a arbitrary player $p_i$, the resulting player vector $v^{(p_i)} \in \mathbb{R}_{\geq 0}^{18}$ is therefore 18-dimensional, containing non-negative values.
...
selected_players = [
    14712,       # Manuel Neuer,         Position: Goalkeeper
    3359,        # Lionel Messi,         Position: Center forward
    14817,       # Robert Lewandowski,   Postion: Center forward
