...
Let $\textbf{X} \in \mathbb{R}_{\ge 0}^{m \times n}$ be an arbitrary matrix, and let $k \in \mathbb{N}$ with $k < \min\{n, m\}$ be the number of components. 
Non-Negative Matrix Factorization (NMF or NNMF) [[2]](#lee2001) finds two matrices $\textbf{W} \in \mathbb{R}^{m \times k}_{\ge 0}$ and $\textbf{H} \in \mathbb{R}^{k \times n}_{\ge 0}$ that minimize the difference of the Frobenius norm:
