...
If we are interested in both the start and end location of an event, we can work with two datasets $\mathcal{D}^{\text{start}}_{e}, \mathcal{D}^{\text{end}}_{e}$ containing the start and end positions. We then can compute the heatmaps in parallel using <a href="#algorithm-1">Algorithm 1</a>.
